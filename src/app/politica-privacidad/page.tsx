...
export const dynamic = 'force-dynamic'; // Forzar renderizado dinámico por el no-store de la API

export const metadata: Metadata = {
  title: 'Política de Privacidad - Francisco Fernández',
  description: 'Información sobre el tratamiento de los datos personales recopilados a través del formulario de contacto del portfolio y CV de Francisco Fernández.',
  robots: {
    index: false,
    follow: true,
  },
...
export default async function PrivacyPolicyPage() {
  // Obtenemos el contenido de la página desde WordPress
  const pageData = await getPageBySlug('/politica-privacidad/');
  ...
  const title = pageData?.title || 'Política de Privacidad';
  ...
  return (
    <DashboardLayout userName="Francisco Fernández">
      <div className="max-w-6xl mx-auto py-12 px-4 md:px-8 space-y-8">
        ...
        <div className="bg-panel border border-panel-border rounded-xl p-6 md:p-10 shadow-lg space-y-6">
          ...
          {/* Renderizado de contenido enriquecido (HTML de WordPress) unificado */}
          <div 
